...
@include('panel.admin.components.top-bar')
    ...
    <div class="grid grid-cols-12 box mt-5">
        <div class="intro-y col-span-12 lg:col-span-6">
            <!-- BEGIN: Form Layout -->
            <form action="{{route('ordenesA.update',$orden->id)}}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div>
                        ...
                        <div class="mt-2">
                            <select data-search="true" data-placeholder="Escribe un nombre" class="tail-select w-full" name="driver" required>
                                ...
                                <option value="{{$driver->id}}" {{$driver->id == $orden->user_id ? 'selected' : '' }}>{{$driver->name}} {{$driver->last_name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
        ...
        <div class="intro-y col-span-12 lg:col-span-6">
            ...
            <div class="mt-3">
                <label>Chassis</label>
                <div class="relative mt-2">
                    <input type="text" pattern="[A-Za-z0-9]+" title="Solo se permiten letras y numeros" name="chassis" id="chassis" class="input pr-12 w-full border col-span-4" placeholder="e.j MATZ946731" onkeyup="mayus(this);" value="{{old('chassis',$orden->chassis)}}" required>
                </div>
            </div>
            <div class="mt-3">
